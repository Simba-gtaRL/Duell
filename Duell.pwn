...
#define DUELL 5 //Bei Dialog-Bugs prüfen ob die DefineIDs mit DefineIDs aus dem Main-Script übereinstimmen, falls ja anpassen.
#define DUELL2 6
#define DUELL3 7
#define DUELL4 8
#define DUELL5 9
...
ocmd:duel(playerid,params[])
{
	...
	if(isnull(string))format(string,128,"Kein Spiel geöffnet");
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	if(dialogid == DUELL)
	{
		if(response == 1)
		{
			...
			for(new d=0;d<20;d++)
			{
				...
				if(listitem == d && DuellInfo[d][DuellErstellt] == 1)
			    {
					...
					{
						ShowPlayerDialog(playerid,DUELL6,DIALOG_STYLE_LIST,"Duell Infos",string,"Löschen","Zurück");
						return 1;
					}
					ShowPlayerDialog(playerid,DUELL6,DIALOG_STYLE_LIST,"Duell Infos",string,"Beitreten","Zurück");
				}
			}
		...
		if(response == 0)
		{
			...
			format(string,sizeof(string),"Map: %s\nWetteinsatz: %s\nWaffen: %s",DuellInfo[DuellID[playerid]][DuellMap],DuellInfo[DuellID[playerid]][DuellWetteinsatz],DuellInfo[DuellID[playerid]][DuellWaffen]);
			ShowPlayerDialog(playerid,DUELL2,DIALOG_STYLE_LIST,"Duell erstellen",string,"Ändern","Erstellen");
		}
		return 1;
	}
	if(dialogid == DUELL2)
	...
	if(dialogid == DUELL5)
	{
		...
		for(new d=0;d<26;d++)
		{
		    if(listitem == d)
		    {
				...
				ShowPlayerDialog(playerid,DUELL7,DIALOG_STYLE_INPUT,"Duell erstellen","Bitte gebe die gewünschte Munitionsanzahl ein.","Bestätigen","Zurück");
			}
  		}
        return 1;
	}
	if(dialogid == DUELL6)
	{
		if(response == 1)
		{
			if(playerid == DuellInfo[DuellIDVorschau[playerid]][DuellErstellerPID])
			{
	            ...
	            CancelSelectTextDraw(playerid);
	            DuellIDVorschau[playerid] = -2;
	            SendClientMessage(playerid,HELLBLAU,"Duell erfolgreich gelöscht.");
				return 1;
			}
